...
def check_if_unique(packet, v_packets):
    return any(packet.source_ip == v_packet.source_ip for v_packet in v_packets)

def parent_start(packet):

    def detect_vulnerable_network_protocols(packet):
        if (
            packet.haslayer('OSPF Hello')
            and 'ospf' in user_var.protocol
            and ospf.detect_if_vulnerable(packet)
        ):
        ...
        if (
            packet.haslayer('EIGRP')
            and 'eigrp' in user_var.protocol
            and eigrp.detect_if_vulnerable(packet)
        ):
        ...
        if (
            packet.haslayer('RIP header')
            and 'rip' in user_var.protocol
            and rip.detect_if_vulnerable(packet)
        ):
        ...
        if (
            packet.haslayer('VRRP')
            and 'vrrp' in user_var.protocol
            and vrrp.detect_if_vulnerable(packet)
        ):
        ...
        if (
            packet.haslayer('HSRP')
            and 'hsrp' in user_var.protocol
            and hsrp.detect_if_vulnerable(packet)
        ):
...
def start_scan():
    ...
    for v_packet in vulnerable_eigrp_packets:
        ...
        print(
            Fore.CYAN
            + Style.BRIGHT
            + f'[+]Detected a vulnerable EIGRP configuration for {v_packet.source_ip}'
        )
    ...
    for v_packet in vulnerable_ospf_packets:
        ...
        if v_packet.authtype == 2:
            print(
                Fore.CYAN
                + Style.BRIGHT
                + f'[+]Detected a vulnerable OSPF configuration for {v_packet.source_ip} but crypto authentication is being used'
            )
            ...
            print(
                Fore.CYAN
                + Style.BRIGHT
                + f'[+]Detected a vulnerable OSPF configuration for {v_packet.source_ip}'
            )
    ...
    for v_packet in vulnerable_rip_packets:
        ...
        print(
            Fore.CYAN
            + Style.BRIGHT
            + f'[+]Detected a vulnerable RIP configuration for {v_packet.source_ip}'
        )
    ...
    for v_packet in vulnerable_vrrp_packets:
        ...
        print(
            Fore.CYAN
            + Style.BRIGHT
            + f'[+]Detected a vulnerable VRRP configuration for {v_packet.source_ip}'
        )
    ...
    for v_packet in vulnerable_hsrp_packets:
        ...
        print(
            Fore.CYAN
            + Style.BRIGHT
            + f'[+]Detected a vulnerable HSRP configuration for {v_packet.source_ip}'
        )
